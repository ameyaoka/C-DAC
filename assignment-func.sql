...
CREATE PROCEDURE GetFullNameInUpperCase (IN first_Name VARCHAR(100), IN last_Name VARCHAR(100))
BEGIN
    DECLARE fullName VARCHAR(200);
    
    SET fullName = CONCAT(UPPER(first_Name), ' ', UPPER(last_Name));
...
CREATE PROCEDURE InsertNewEmployee (IN newFirstName VARCHAR(100), IN newLastName VARCHAR(100), IN departmentID INT)
BEGIN
    INSERT INTO employees (first_name, last_name, department_id)
    VALUES (newFirstName, newLastName, departmentID);
END //

DELIMITER ;

CALL InsertNewEmployee('John', 'Doe', 123);
...
#8 develope  a stored procedure that accepts an employee ID as input and deletes that employee's record from the Employees table.
